...
export class SalesDetailsService {
  ...
  async createSaleDetail(createSalesDetailDtos: CreateSalesDetailDto[], transactionalEntityManager: EntityManager){
    
    for (const dto of createSalesDetailDtos) {
      const productExists = await this.productRepository.findOne({ where: { code: dto.code } });
      if (!productExists) {
